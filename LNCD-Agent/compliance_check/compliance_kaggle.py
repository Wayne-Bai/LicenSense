...
def analyze_downstream_usage(downstream_datasets, original_info):
    ...
    for idx, dataset_record in enumerate(downstream_datasets, 1):
        ...
        print(f"Processing record {idx}/{total}: {dataset_record.get('ref')}")
...
def check_kaggle(KEYWORD):
    ...
    with open(here(f"final_processed_kaggle_{KEYWORD}.json"), "r") as f:
    ...
    save_violations_to_file(updated_records, f"violations_kaggle_{KEYWORD}.json")
    print(f"Analysis complete. Updated records saved to violations_kaggle_{KEYWORD}.json")

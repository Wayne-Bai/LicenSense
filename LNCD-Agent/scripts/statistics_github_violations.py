...
def collect_files(keyword: str, base_dir: Path | None = None) -> List[Path]:
    ...
    Collect JSON files from the 'main entrance' directory (current working dir via $PWD or os.getcwd()).
      ...
      - Else => all files matching violations_github_*.json.
    """
    if base_dir is None:
        base_dir = Path(os.environ.get("PWD", os.getcwd())).resolve()

    if keyword:
        fname = f"violations_github_{safe_name(keyword)}.json"
        candidate = base_dir / fname
        if not candidate.exists():
            raise FileNotFoundError(f"Expected file not found: {candidate}")
        return [candidate]
    else:
        files = sorted(base_dir.glob("violations_github_*.json"))
        if not files:
            raise FileNotFoundError(
                f"No files found in {base_dir} matching: violations_github_*.json"
            )
        return files

def infer_keyword_from_filename(path: Path) -> str:
    """
    Extract the keyword from a filename like 'violations_github_<keyword>.json'.
    Returns the sanitized keyword part (whatever is in the filename).
    """
    stem = path.stem  # e.g., 'violations_github_Adience'
    prefix = "violations_github_"
...
def normalize_items(data) -> list:
    ...
    Accepts either:
      ...
      - a dict with a list under common keys (repos/items/data),
...
def count_rule_violations(items: list) -> Counter:
    """
    Count violation entries per RULE across all repos in the list.

...
def collect_files(keyword: str, base_dir: Path | None = None) -> List[Path]:
    ...
    Collect JSON files from the 'main entrance' directory (current working dir via $PWD or os.getcwd()).
      ...
      - Else => all files matching final_processed_kaggle_*.json.
    """
    if base_dir is None:
        base_dir = Path(os.environ.get("PWD", os.getcwd())).resolve()

    if keyword:
        fname = f"final_processed_kaggle_{safe_name(keyword)}.json"
        candidate = base_dir / fname
        if not candidate.exists():
            raise FileNotFoundError(f"Expected file not found: {candidate}")
        return [candidate]
    else:
        files = sorted(base_dir.glob("final_processed_kaggle_*.json"))
        if not files:
            raise FileNotFoundError(
                f"No files found in {base_dir} matching: final_processed_kaggle_*.json"
            )
        return files

def infer_keyword_from_filename(path: Path) -> str:
    """
    Extract the keyword from a filename like 'final_processed_kaggle_<keyword>.json'.
    Returns the sanitized keyword part (whatever is in the filename).
    """
    stem = path.stem  # e.g., 'final_processed_kaggle_BRSET'
    prefix = "final_processed_kaggle_"
